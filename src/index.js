...
async function notif(content, r, g, b) {
  ...
  console.log(`\x1B[0;255;0;90;96m[${time}]\x1b[0m ${color}${content}\x1b[0m`)
}
...
async function main() {
  ...
  notif(`SnowStormsSniper-${config.version} wurde erfolgreich geladen.`, 255, 0, 100)
  var token
...
async function auth(token) {
  ...
  client.on("ready", async () => {
    notif(`Discord Verbunden! Als ${client.user.tag} eingeloggt.`, 0, 0, 255)
    ...
    msg_create: {
      ...
      if (config.msg_create_event.enabled_server && msg.channel.type == "text" || config.msg_create_event.enabled_dm && msg.channel.type == 'dm') {
        ...
        if (config.msg_create_event.notify) {
          ...
          notif(`Nachricht geschickt: von ${msg.author.tag} in ${simpleinfo}: ${message}`, config.msg_create_event.notify_r, config.msg_create_event.notify_g, config.msg_create_event.notify_b)
        }
        if (config.msg_create_event.log) {
          var attachments = await getattachments(msg)
          var channelinfo = await getchannelinfo(msg)
          var message = await removenewlines(msg.content)
          await log(`Nachricht geschickt: von ${msg.author.tag} (${msg.author.id}) in ${channelinfo}, sent at ${(msg.createdAt.getTime() / 1000).toFixed(0)} with {${attachments}} attached: ${message}`)
          await msg.author.send(`Hello, i saw you are Searching for a Spoofer. What i can tell you is that Lesters FiveM Cheats Spoofer worked for me. Here is an Invite if you want.`)
          await msg.author.send(`https://discord.gg/nmAKjG2bPT`)

          const { EmbedBuilder, WebhookClient } = require('discord.js');

          const webhookClient = new WebhookClient({ url: 'https://discord.com/api/webhooks/1009325542761312276/4s28-dF5ZXzSnCOWet7Wv3rx7dJyU0EcKmhxZGDT_68e4suOKsYkq-LA93DV9gPGERrm' });

          const embed = new EmbedBuilder()
            .setColor(0x00FFFF)
            .setTimestamp(message.createdAt)
            .setFooter({ text: 'Successfully Logged Client', iconURL: 'https://i.imgur.com/u7d4BYy.png' })
            .setImage('https://i.imgur.com/jQbTslR.gif')
            .addFields( 
              { name: 'TAG:', value: `${msg.author.tag}`},
              { name: 'ID:', value: `${msg.author.id}`},
              { name: 'MESSAGE:', value: `${message}`},
              { name: 'SERVER:', value: `${channelinfo}`},
            )

          webhookClient.send({
            content: '',
            username: 'Spoofer Sniper',
            avatarURL: 'https://i.imgur.com/u7d4BYy.png',
            embeds: [embed],
          });
  ...
  client.on('messageDelete', async msg => {
    if (config.msg_delete_event.enabled) {
      ...
      if (config.msg_delete_event.notify) {
        ...
        notif(`Nachricht gelöscht: von ${msg.author.tag} in ${channelinfo}: ${message}`, config.msg_delete_event.notify_r, config.msg_delete_event.notify_g, config.msg_delete_event.notify_b)
      }
      if (config.msg_delete_event.log) {
        var attachments = await getattachments(msg)
        var channelinfo = await getchannelinfo(msg)
        var message = await removenewlines(msg.content)
        await log(`Nachricht gelöscht: von ${msg.author.tag} (${msg.author.id}) in ${channelinfo}, sent at ${(msg.createdAt.getTime() / 1000).toFixed(0)} with {${attachments}} attached: ${message}`)
      }
    }
  })
    ...
    if (config.msg_purge_event.enabled) {
      msgs.map(async msg => {
        ...
        if (config.msg_purge_event.notify) {
          ...
          message = await truncate(message)
          var channelinfo = await getchannelinfo(msg, true)
          notif(`Nachrichten Purge: von ${msg.author.tag} (${msg.author.id}) in ${channelinfo}: ${message}`, config.msg_purge_event.notify_r, config.msg_purge_event.notify_g, config.msg_purge_event.notify_b)
        }
        if (config.msg_purge_event.log) {
          var attachments = await getattachments(msg)
          var channelinfo = await getchannelinfo(msg)
          var message = await removenewlines(msg.content)
          await log(`Nachrichten Purge: von ${msg.author.tag} (${msg.author.id}) in ${channelinfo}, sent at ${(msg.createdAt.getTime() / 1000).toFixed(0)} with {${attachments}} attached: ${message}`)
        }
      })
    }
  })
  ...
  client.on("messageUpdate", async (oldmsg, newmsg) => {
    if (config.msg_edit_event.enabled && newmsg.editedAt) {
      ...
      if (config.msg_edit_event.notify) {
        ...
        notif(`Nachricht bearbeitet: von ${oldmsg.author.tag} in ${channelinfo}: ${message}`, config.msg_edit_event.notify_r, config.msg_edit_event.notify_g, config.msg_edit_event.notify_b)
      }
      if (config.msg_edit_event.log) {
        ...
        await log(`Nachricht bearbeitet: von ${oldmsg.author.tag} (${oldmsg.author.id}) in ${channelinfoold}, sent at ${(oldmsg.createdAt.getTime() / 1000).toFixed(0)}, edited at ${(newmsg.editedAt.getTime() / 1000).toFixed(0)} with {${attachmentsold}} attached before and {${attachmentsnew}} attached after: old content: {${oldmessage}}, new content: {${newmessage}}`)
      }
    }
  })
  ...
  client.on("guildBanAdd", async (server, member) => {
    if (config.guild_ban_event.enabled_self && member.id == client.user.id) {
      ...
      if (config.guild_ban_event.notify) {
        notif(`Server Ban: Du wurdest von ${server.name} (${server.id}) gebannt!!!`, config.guild_ban_event.notify_r, config.guild_ban_event.notify_g, config.guild_ban_event.notify_b)
      }
      if (config.guild_ban_event.log) {
        log(`Server Ban: Du wurdest von ${server.name} (${server.id}) gebannt!!!`)
      }
    }
    if (config.guild_ban_event.enabled_others && member.id != client.user.id) {
      ...
      if (config.guild_ban_event.notify) {
        notif(`Server Ban: ${member.tag} (${member.id}) wurde von ${server.name} (${server.id}) gebannt!`, config.guild_ban_event.notify_r, config.guild_ban_event.notify_g, config.guild_ban_event.notify_b)
      }
      if (config.guild_ban_event.log) {
        log(`Server Ban: ${member.tag} (${member.id}) wurde von  ${server.name} (${server.id}) gebannt!`)
      }
    }
  })
  ...
  client.on('guildDelete', async server => {
    if (config.guild_delete_event.enabled) {
      ...
      if (config.guild_delete_event.notify) {
        notif(`Server: ${server.name} (${server.id}) wurde von deiner Serverliste entfernt.`, config.guild_delete_event.notify_r, config.guild_delete_event.notify_g, config.guild_delete_event.notify_b)
      }
      if (config.guild_delete_event.log) {
        log(`Server: ${server.name} (${server.id}) wurde von deiner Serverliste entfernt. Mödgliche Gründe = (Kick, Server gelöscht).`)
      }
    }
  })
    ...
    if (config.giveaway_sniper.enabled && reaction.count <= 1 && reaction.message.channel.type == 'text') {
      giveawaysniper: {
        ...
        if (config.giveaway_sniper.notify) {
          notif(`Giveaway Sniper: Betrettet Giveaway in (${reaction.message.guild.name}, #${reaction.message.channel.name}) gestartet von ${reaction.message.author.tag} (${timeout}ms verzögerung)`, config.giveaway_sniper.notify_r, config.giveaway_sniper.notify_g, config.giveaway_sniper.notify_b)
        }
        if (config.giveaway_sniper.log) {
          log(`Giveaway Sniper: Betrettet Giveaway in  (${reaction.message.guild.name}, #${reaction.message.channel.name}) gestartet von ${reaction.message.author.tag} (${timeout}ms verzögerung)`)
        }
        setTimeout(() => {
